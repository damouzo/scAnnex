...
if [ -f "test_data/samplesheet_slc_test.csv" ]; then
    echo -e "  ${GREEN}✓ Samplesheet found${NC}"
    if [ -d "test_data/mtx/filtered_feature_bc_matrix" ]; then
        MATRIX=$(ls test_data/mtx/filtered_feature_bc_matrix/matrix.mtx.gz 2>/dev/null)
        if [ -n "$MATRIX" ]; then
